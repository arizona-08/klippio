...
function VisualizerMenu() {
  ...
  return (
    <div className={`w-12 bg-gray-200 rounded-full  cursor-pointer fixed bottom-12 right-5 z-50 ${isMenuOpen ? 'h-45' : 'h-12'} transition-all duration-150 overflow-hidden`}>
      <div className="absolute top-0 left-0 w-full elements flex flex-col items-center gap-4 h-50">
        <div className="bg-red-500 w-full h-5"></div>
        <div className="bg-red-500 w-full h-5"></div>
        <div className="bg-red-500 w-full h-5"></div>
      </div>
      <div className="burgerMenu absolute bottom-4 w-12 h-4 px-6 cursor-pointer" onClick={toggleMenu}>
