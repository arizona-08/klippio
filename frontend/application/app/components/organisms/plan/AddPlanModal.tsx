...
function AddPlanModal({
  ...
  const isDisabled =
    plan == null || plan?.name.trim() === "" || plan?.file === null;

  function handleChangePlanTitle(event: React.ChangeEvent<HTMLInputElement>) {
    setPlan((prevPlan) =>
      prevPlan
        ? { ...prevPlan, name: event.target.value }
        : { name: event.target.value, file: null as unknown as File },
    );
  }
  ...
  function handleChangePlanPDF(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    const file = files[0];
    setPlan((prevPlan) =>
      prevPlan ? { ...prevPlan, file } : { name: "", file },
    );
  }
  ...
  return (
    <div
      className={`bg-black/25 backdrop-blur-sm fixed inset-0 z-70 flex items-center justify-center p-4 ${isActive ? "block" : "hidden"}`}
    >
          ...
          {plan && plan.file && (
            <div className="flex items-center gap-2 py-1 px-2 border border-primary rounded-full w-fit mt-3">
              <p className="text-sm text-gray-600">
                {plan.file.name} {convertFileSizeInMo(plan.file.size)} Mo
              </p>
              <X
                ...
                onClick={() => {
                  setPlan((prevPlan) =>
                    prevPlan
                      ? { ...prevPlan, file: null as unknown as File }
                      : null,
                  );
