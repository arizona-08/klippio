...
export class AmazonS3Service {
  ...
  async uploadImage({
    ...
    const bucketName = this.configurationService.getOrThrow<string>(
    ...
    try {
      ...
      const temporaryAccessUrl = await this.generatePresignedUrl(
        storageKey,
        3600,
      );
      ...
      this.loggerInstance.error(
        "Échec lors de l'envoi de l'image sur Amazon S3",
        uploadError,
      );
      throw uploadError;
    }
  ...
  async generatePresignedUrl(
    ...
    try {
      ...
      throw new InternalServerErrorException(
        "Impossible de générer l'accès au fichier sécurisé",
      );
    }
  }
  ...
  async deleteImage(storageKey: string) {
    ...
    try {
      ...
      this.loggerInstance.error(
        "Erreur lors de la suppression de l'image sur Amazon S3",
        deleteError,
      );
      throw new InternalServerErrorException(
        'Impossible de supprimer le fichier sur Amazon S3',
      );
    }
  }
}
